...
import { FileDown, Share2, BookOpen, Bold, Italic, List, ListOrdered, AlignLeft, Undo, Redo, LinkIcon, HelpCircle, FileText } from "lucide-react"
...
export default function DidacticSequenceGenerator() {
  ...
  // Efecto para limpiar el chat cuando cambia la secuencia
  useEffect(() => {
    if (sequence) {
      // Solo limpiar si el cambio viene de una nueva generación, no del chat
      const isFromChat = sessionStorage.getItem('secuenciaFromChat')
      if (!isFromChat) {
        localStorage.removeItem('secuenciaChatState')
        console.log('Secuencia generada desde formulario, chat limpiado')
      } else {
        // Limpiar la marca de que viene del chat
        sessionStorage.removeItem('secuenciaFromChat')
      }
    }
  }, [sequence])
  ...
  const getCurrentContent = () => {
    // Retornar el contenido actual de la secuencia
    return sequence || ""
  }
  ...
  const handleContentUpdate = (newContent: string) => {
    try {
      ...
      sessionStorage.setItem('secuenciaFromChat', 'true')
      
      // Actualizar el contenido de la secuencia
      setSequence(newContent)
      setEditedContent(newContent)
      toast.success("Secuencia didáctica actualizada exitosamente")
    } catch (error) {
      console.error('Error al procesar la respuesta del chat:', error)
      toast.error("Error al actualizar la secuencia didáctica.")
    }
  }
 ...
 if (!accessData.allowed) {
  ...
  return // Detener el flujo
      ...
      // Limpiar el estado del chat cuando se genera una nueva secuencia
      localStorage.removeItem('secuenciaChatState')
    ...
    if (!email) {
      ...
      await gamificationService.addPoints(email, 10, 'didactic_sequence_generation')
      showPointsNotification(10, 'generar una secuencia didáctica')

  }

  const handleInputChange = (field: keyof SequenceFormData, value: string | boolean) => {
  ...
  const exportToPDF = async () => {
    ...
    try {
      ...
      pdf.text(`Secuencia Didáctica: ${formData.topic}`, marginLeft, marginTop);
      ...
      // Metadatos
      pdf.setProperties({
        title: `Secuencia Didáctica - ${formData.topic}`,
        subject: formData.course,
        author: 'Generador de Secuencias',
      });

      // Guardar PDF
      pdf.save(`secuencia-${formData.topic.toLowerCase().replace(/\s+/g, '-')}.pdf`);
      toast.success("Secuencia didáctica exportada exitosamente");
    } catch (error) {
      console.error('Error al generar PDF:', error);
      toast.error("Error al exportar la secuencia didáctica");
    }
  };

  const handleExportWord = async () => {
    if (!editedContent) {
      toast.error('No hay secuencia didáctica para exportar');
      return;
    }
    ...
    try {
      await exportToWord(
        editedContent,
        `Secuencia Didáctica: ${formData.topic}`,
        `secuencia-${formData.topic.toLowerCase().replace(/\s+/g, '-')}`
      );
  ...
  const shareSequence = async () => {
    try {
      if (navigator.share) {
        ...
        toast.success("Secuencia didáctica copiada al portapapeles");
      }
    } catch (error) {
      console.error('Error al compartir:', error);
      toast.error("Error al compartir la secuencia didáctica");
    }
  };
    ...
    <Layout>
      ...
      >
        ...
        >
          ...
          >
            <div className="flex items-center gap-2">
              ...
              >
                <h1 className="text-xl font-semibold">Generador de Secuencia Didáctica</h1>
              </motion.div>
              <Dialog>
          ...
          {sequence ? (
            ...
            >
              ...
              <p className="text-center mb-2">No hay secuencia didáctica generada</p>
              <p className="text-center text-sm">
                Configura el tema en el panel derecho y genera una secuencia didáctica detallada automáticamente.
              </p>
            </motion.div>
          )}
        </motion.div>
        ...
        >
          ...
          >
            <h2 className="text-xl font-semibold mb-6">Generador de Secuencia Didáctica</h2>
              ...
              >
                ...
                >
                  ...
                  <Input
                    ...
                    placeholder="Ej: Secuencia Didáctica de Programación Básica"
                    required
                    maxLength={100}
                  />
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <TooltipProvider>
                      <Tooltip>
                        ...
                        <TooltipContent>
                          <p>Describe el tema principal para el que deseas generar la secuencia didáctica.</p>
                        </TooltipContent>
                      </Tooltip>
                    </TooltipProvider>
                  </div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <TooltipProvider>
                      <Tooltip>
                        ...
                        <TooltipContent>
                          <p>Selecciona el nivel educativo para adaptar la secuencia didáctica.</p>
                        </TooltipContent>
                      </Tooltip>
                    </TooltipProvider>
                  </div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <TooltipProvider>
                      <Tooltip>
                        ...
                        <TooltipContent>
                          <p>Al marcar esta opción, tu secuencia didáctica será visible para otros educadores en la comunidad.</p>
                        </TooltipContent>
                      </Tooltip>
                    </TooltipProvider>
                  </div>
        ...
        <Dialog open={showPreviewModal} onOpenChange={setShowPreviewModal}>
          <DialogContent className="max-h-[90vh] overflow-y-auto w-[95vw] sm:w-[90vw] md:w-[80vw] lg:w-[70vw] xl:w-[60vw] p-0">
            <DialogHeader className="px-6 pt-6 pb-4">
              <DialogTitle>Secuencia Didáctica</DialogTitle>
            </DialogHeader>
        ...
        <ChatImprovementModal
          ...
          title="Asistente de Mejora de Secuencia Didáctica"
          description="Solicita mejoras específicas para tu secuencia didáctica. El asistente te ayudará a optimizar la estructura, actividades y efectividad de la secuencia."
          suggestions={[
            "Mejora la estructura de la secuencia",
            "Añade más actividades de desarrollo",
            "Incluye actividades de evaluación",
            "Mejora las instrucciones de las actividades",
            "Añade actividades de cierre",
            "Incluye recursos adicionales"
          ]}
          localStorageKey="secuenciaChatState"
          isSlidesMode={false}
        />
      </motion.div>
      <Toaster />
